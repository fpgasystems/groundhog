 ...
 * ---------------------------------------------------------------------------
 * Description   : Buffers used in the transport layer (and also test module) 
 *                 to buffer incoming/outgoing data
 * ---------------------------------------------------------------------------
 * Changelog     : none
 * ------------------------------------------------------------------------- */
...
module FIFO
  #(
    ...
    parameter UNDERRUNLIM = 2,
    parameter OVERFLOWLIM = 6
    )
   ( 
     input                    clk,
     input                    reset,
   ...
   // make an DEPTH-deep array of WIDTH-sized registers
   reg [(WIDTH-1)     : 0] mem [0:(DEPTH-1)];
   reg [(ADDR_BITS-1) : 0] readAdd;
   reg [(ADDR_BITS-1) : 0] writeAdd;
   ...
   always@(posedge clk) begin
      if(reset)begin
         ...
         if((!internal_full) && (dataInWrite == 1)) begin
            writeAdd      <= writeAdd + 1;
            mem[writeAdd] <= dataIn;
            if((dataOutRead == 0) || (empty)) begin
               count <= count+1;
            end
         end
         if((!empty) && (dataOutRead == 1)) begin
            readAdd <= readAdd + 1;
            if((dataInWrite == 0) || (internal_full)) begin
               count <= count-1;
            end
         end
      end
   ...
   assign dataOut       = mem[readAdd];
   assign empty         = count == 0;
   assign internal_full = (count == DEPTH);
   assign full          = internal_full;
   assign underrun      = count < UNDERRUNLIM;
   assign overflow      = count > OVERFLOWLIM;
